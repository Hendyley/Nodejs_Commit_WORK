...
case when EOH.WorkstationGroupName is null then ' ' else EOH.WorkstationGroupName end as WorkstationGroupName,
--wip.[Part Group],
...
(
select distinct
EOH.WorkstationGroupName,
EOH.TechNode,
EOH.StepName,
EOH.AreaName,
case 
when EOH.TechNode  like 'N100S' then '100S'
when EOH.TechNode  like 'N110S' then '110S'
when EOH.TechNode  like 'N120S' then '120S'
when EOH.TechNode  like 'N130S' then '130S'
when EOH.TechNode  like 'N140S' then '140S'
when EOH.TechNode  like 'N150S' then '150S'
when EOH.TechNode  like 'N160S' then '160S'
when EOH.TechNode  like 'N170S' then '170S'
else null end as Technode_S
from OPS_IMP_METRICS.OI_METRICS.MfgEOHTechNodeSummary EOH 
where
EOH.WorkstationGroupName is not null and EOH.AreaName is not null
) EOH
on EOH.StepName = wip.[Step Name]  and wip.[Area Name] = EOH.AreaName and wip.[Tech Node] = EOH.Technode_S
--inner join #wsg wsg on wsg.WS_group_name = EOH.WorkstationGroupName
--left join #sim s
